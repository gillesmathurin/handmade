...
<?php
  ...
  //       $output.='<<article class="caption">';
  //       $k = 1;
  //       foreach ($items as $item ){
  //           	$output.='<a class="colorbox" title="' . $item->item_description . '" href="' . $item->item_image . '"><img alt="" class="caption__media" src="' . $item->item_thumbnail . '"></a>';
  //                   if($item->item_title != null) { $output.='<dd><strong>' . $item->item_title . '</strong></dd>'; }
		// 			if($item->item_client != null) { $output.='<dd>' . $item->item_client . '</dd>'; }
		// 			if($item->item_link != null) { $output.='<dd><a target="_' . $afpOptions['project_link'] . '" href="' . $item->item_link . '">' . __('Project Link', 'awesome-filterable-portfolio') . '</a></dd>'; }
  //               $output.='';
  ...
  //       $output.='</article>
        ...
        foreach ($items as $item ){
              $output.='<li class="afp-single-item" data-id="id-' . $k . '" data-type="' . ereg_replace("[^A-Za-z0-9]", "", $item->item_category) .'">
                <a class="colorbox" title="' . $item->item_description . '" href="' . $item->item_image . '"><img alt="" class="img-link-initial" src="' . $item->item_thumbnail . '"></a>
                <div class="afp-item-details">';
                    if($item->item_title != null) { $output.='<div><strong>' . $item->item_title . '</strong></div>'; }
          if($item->item_client != null) { $output.='<div>' . $item->item_client . '</div>'; }
          if($item->item_date != '0000-00-00') { $output.='<div>' . date("m/d/Y", strtotime($item->item_date)) . '</div>'; }
          if($item->item_link != null) { $output.='<div><a target="_' . $afpOptions['project_link'] . '" href="' . $item->item_link . '">' . __('Project Link', 'awesome-filterable-portfolio') . '</a></div>'; }
                $output.='</div>
            </li>';
